...
/**
 * This class tests the UnorderedArrayMap class for proper implementation of the data structure.
...
public class UnorderedArrayMapTest {
	...
	public void testPut() {
		...
		assertTrue(map.get(3).contentEquals("string3"));
		
		assertFalse(map.isEmpty());
		assertNull(map.put(100, "string100"));
		assertEquals("UnorderedArrayMap[100, 3]", map.toString());
		assertEquals(2, map.size());
		assertTrue(map.get(100).contentEquals("string100"));
		
		assertFalse(map.isEmpty());
		assertEquals(map.put(3, "newString3"), "string3");
		assertEquals("UnorderedArrayMap[3, 100]", map.toString());
		assertEquals(2, map.size());
	...
	public void testRemove() {
		...
		assertFalse(map.isEmpty());
		assertEquals("UnorderedArrayMap[1, 2, 5, 3]", map.toString());
		
		assertEquals(map.get(1), map.remove(1));
		assertEquals(3, map.size());
		assertFalse(map.isEmpty());
		assertEquals("UnorderedArrayMap[2, 5, 3]", map.toString());
		
		assertEquals(map.get(3), map.remove(3));
		assertEquals(2, map.size());
		assertFalse(map.isEmpty());
		assertEquals("UnorderedArrayMap[2, 5]", map.toString());
